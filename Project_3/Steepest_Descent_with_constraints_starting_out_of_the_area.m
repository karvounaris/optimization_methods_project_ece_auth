function [current_point, value_of_function, number_of_loops] = Steepest_Descent_with_constraints_starting_out_of_the_area(f, starting_point, gk, epsilon, x_constraints, y_constraints, sk)
    ...
    %initialize starting point and checks if it is out of the area of the
    %constraints. If it is not use the projection method on the starting
    %point and then start the algorithm
    current_point = starting_point;
    %function's gradient
    %use of the contraints
    %xi = ai if xi <= ai
    if current_point(1,1) <= x_constraints(1)
        current_point(1,1) = x_constraints(1);
    %xi = bi if xi >= bi
    elseif current_point(1,1) >= x_constraints(2)
        current_point(1,1) = x_constraints(2);
    end
       
    %xi = ai if xi <= ai
    if current_point(2,1) <= y_constraints(1)
        current_point(2,1) = y_constraints(1);
    %xi = bi if xi >= bi
    elseif current_point(2,1) >= y_constraints(2)
        current_point(2,1) = y_constraints(2);
    end
